...
import { LogOut, Activity, User, Heart, Calendar, FileSpreadsheet } from 'lucide-react';
...
export default function PatientLayout() {
  ...
  const menuItems = [
    { path: '/patient/profile', label: 'My Health Profile', icon: User },
    { path: '/patient/vitals', label: 'My Live Vitals', icon: Heart },
    { path: '/patient/treatments', label: 'Treatment Timeline', icon: Calendar },
    { path: '/patient/simulations', label: 'My Simulator Projections', icon: FileSpreadsheet },
  ];
      ...
      <aside className="w-full md:w-64 bg-slate-900/40 backdrop-blur-md border-b md:border-b-0 md:border-r border-slate-900 p-4 flex flex-col justify-between shrink-0">
        <div className="space-y-6">
          ...
          <div className="flex items-center gap-2 px-2 py-3 border-b border-slate-900">
            ...
            <span className="font-bold text-sm bg-gradient-to-r from-violet-400 to-indigo-400 bg-clip-text text-transparent tracking-wide">
              MY ONCO-PORTAL
            </span>
          </div>
        ...
        {/* Patient Identity & Logout */}
        <div className="mt-8 pt-4 border-t border-slate-900 space-y-4">
          <div className="flex items-center gap-2.5 px-2">
            <div className="w-8 h-8 rounded-full border border-slate-800 bg-slate-950 flex items-center justify-center text-slate-400">
              <User className="w-4 h-4" />
            </div>
            <div className="truncate">
              <div className="text-xs font-bold text-slate-200 truncate">{user?.name}</div>
              <div className="text-[9px] text-slate-500 truncate">ID: {user?.id}</div>
            </div>
          </div>
